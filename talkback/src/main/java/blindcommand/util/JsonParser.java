...
//				}
	...
	public static List<SpeechResult> parseGrammarResult(String json, String engType) {
		List<SpeechResult> ret = new ArrayList<>();
		String result = "";
		int sc = 0;
		try {
			JSONTokener tokener = new JSONTokener(json);
...
//					} else {
					...
					for (int j = 0; j < items.length(); j++) {
						...
						sc = obj.getInt("sc");
						ret.add(new SpeechResult(result, sc));
					}
				}
		} catch (Exception e) {
			e.printStackTrace();
